...
Commands that **write files** (`listen -l`, `steg-extract`, `ssh -l`, etc.) require `case-set <room>` first. Details → [COMMAND.en.md](COMMAND.en.md) (workspace / room).
...
Playbooks → [CHEATSHEET.en.md](CHEATSHEET.en.md). Command reference → [COMMAND.en.md](COMMAND.en.md).
...
**File output errors** — Did you run `listen -l` etc. before `case-set <room>`? Without a room, set `CASE_LOOSE=1` → `cases/_unscoped/` ([COMMAND.en.md](COMMAND.en.md)).
