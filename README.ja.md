...
追跡対象のテンプレートからローカル用の Docker Compose 設定を作成し、`.env.example` を `.env` にコピーして、上書きしたい値（`VNC_PASSWORD`、必要なら `WPSCAN_API_TOKEN`、`TOOLKIT_LANG` など）を編集する。`docker-compose.yml` はローカル専用で、Git 管理外。
...
`kali` サービスが TryHackMe 用のメイン環境。`services/` 以下のその他のサービスは、コマンドやネットワークツールをテストするための対象サービスで、必要な場合はローカルの `docker-compose.yml` で有効化する。
...
`listen -l` / `steg-extract` / `ssh -l` など、ファイルへ書き出すコマンドは、先に `case-set <room>` を実行すること。詳細 → [COMMAND.ja.md](COMMAND.ja.md) の workspace / ルーム。
...
定番手順 → [CHEATSHEET.ja.md](CHEATSHEET.ja.md)。コマンド詳細 → [COMMAND.ja.md](COMMAND.ja.md)。

## Repository

```
├── docker-compose.yml
├── docker-compose.yml.example  追跡対象の Compose テンプレート
├── services/              サービス定義（`kali` とコマンドテスト用サービス）
...
**ファイル出力エラー** — `case-set <room>` 前に `listen -l` 等を実行していないか。未設定時は `CASE_LOOSE=1` → `cases/_unscoped/`（[COMMAND.ja.md](COMMAND.ja.md)）。
