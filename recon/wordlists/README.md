...
- **UI からだけ選ぶ:** 運用時は scout（`-w` ピッカー）かカタログ id で選ぶ。`/usr/share/seclists/...` を直接 `-w` に書かない想定。
...
## 並列 dir（`scout -ds`）— preset tier

**`-ds` 省略** = **standard** tier まで（累積）。**`-p next`** = 次 tier の `adds` だけ（済み job はスキップ）。
...
| standard | + raft-small-directories（**default**） |
...
|------|----------------|
| light | common |
| standard | + dirbuster-small（**default**） |
| wide | + dirbuster-medium |
| deep | + raft-small-files |

**aliases:** `fast→light`, `ctf→standard`。旧 dirs `-p deep`（4 jobs）は **`wide`** を使用。
